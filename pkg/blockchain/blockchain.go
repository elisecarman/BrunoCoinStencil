...
func (bc *Blockchain) Add(b *block.Block) {
	...
	for _, trx := range b.Transactions {
		for _, inpt := range trx.Inputs {
			loc := txo.MkTXOLoc(inpt.TransactionHash, inpt.OutputIndex)
			delete(utxoUpdate, loc)
		}
		h := trx.Hash()
		index := uint32(0)
		for _, outpt := range trx.Outputs {
			loc := txo.MkTXOLoc(h, index)
			utxoUpdate[loc] = outpt
			index += 1
		}
	}
...
func (bc *Blockchain) GetUTXOForAmt(amt uint32, pubKey string) ([]*UTXOInfo, uint32, bool) {
	bc.Lock()
	defer bc.Unlock()
	if bc.GetBalance(pubKey) >= amt {  // maybe a better way to check pubKey <<-- removed pub key string
		var info []*UTXOInfo
