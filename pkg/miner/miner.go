...
func (m *Miner) HndlBlk(b *block.Block) {
	if b == nil {
		fmt.Printf("ERROR {m.HndlBlk}: "+
			"nil transactions were given to the function")
		return
	}
	//Should we check that the new added block is part of a fork or the main chain?
	//if not:
	//if b.Hdr.PrvBlkHsh == m.PrvHsh  --. NAH
	//compare depth? there is no way to access other branches here
	m.SetHash(b.Hash()) //Should we check that prevHash of block corresponds to Hash of miner?
	m.IncChnLen()
	// here remove some transactions from transaction pool
	m.HndlChkBlk(b)

	// figure out what to do with else statement??
}
...
func (m *Miner) HndlChkBlk(b *block.Block) {
	if b == nil {
		fmt.Printf("ERROR {tp.HndlChkBlk}: "+
			"nil transactions were given to the function")
		return
	}
...
func (m *Miner) HndlTx(t *tx.Transaction) {
	//utils.Debug.Printf("Mnr.HndlBlk1: the Sum input of the first transaction %v of mining pool is %v",
	//	t.NameTag(),t.SumInputs())
	if t != nil {
		m.TxP.Add(t)
		if !m.Mining.Load() && m.TxP.PriMet() { //how to check for met threshold? What about met maximum count?
			m.Mining.Store(true)
		}
